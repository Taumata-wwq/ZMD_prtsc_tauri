...
use crate::models::setting::default_settings;
use crate::models::window_state::WindowState;
use crate::services::seed_data::default_regions;

const REGION_SELECT_FIELDS: &str = "id, name, category, aspect_ratio, scroll_mode, \
    grid_rows, grid_cols, overlap_x, overlap_y, drag_x, drag_y, \
    capture_region_x, capture_region_y, capture_offset_y, template_ref, \
    target_w, target_h, sub_map, created_at, updated_at";

const REGION_INSERT_FIELDS: &str = "name, category, aspect_ratio, scroll_mode, grid_rows, \
    grid_cols, overlap_x, overlap_y, drag_x, drag_y, \
    capture_region_x, capture_region_y, capture_offset_y, template_ref, \
    target_w, target_h, sub_map, created_at, updated_at";

fn row_to_region_config(row: &rusqlite::Row<'_>) -> rusqlite::Result<RegionConfig> {
    Ok(RegionConfig {
        id: Some(row.get(0)?), name: row.get(1)?, category: row.get(2)?,
        aspect_ratio: row.get(3)?, scroll_mode: row.get(4)?,
        grid_rows: row.get(5)?, grid_cols: row.get(6)?,
        overlap_x: row.get(7)?, overlap_y: row.get(8)?,
        drag_x: row.get(9)?, drag_y: row.get(10)?,
        capture_region_x: row.get(11)?, capture_region_y: row.get(12)?,
        capture_offset_y: row.get(13)?, template_ref: row.get(14)?,
        target_w: row.get(15)?, target_h: row.get(16)?, sub_map: row.get(17)?,
        created_at: row.get(18)?, updated_at: row.get(19)?,
    })
}

fn row_to_session(row: &rusqlite::Row<'_>) -> rusqlite::Result<CaptureSession> {
    Ok(CaptureSession {
        id: Some(row.get(0)?), started_at: row.get(1)?, finished_at: row.get(2)?,
        region: row.get(3)?, scroll_mode: row.get(4)?,
        grid_rows: row.get(5)?, grid_cols: row.get(6)?, total_shots: row.get(7)?,
        status: row.get(8)?, original_path: row.get(9)?, exported_path: row.get(10)?,
        thumbnail_path: row.get(11)?, crop_box: row.get(12)?,
        output_format: row.get(13)?, jpg_quality: row.get(14)?,
    })
}
...
impl DbState {
    ...
    fn seed_defaults(&self) -> AppResult<()> {
        ...
        conn.execute(
            "INSERT OR IGNORE INTO window_state (id, width, height, is_maximized, always_on_top, updated_at) VALUES (1, 900, 600, 0, 0, ?)",
            params![now],
        )?;
        Ok(())
    }
    ...
    fn insert_default_regions(conn: &Connection, now: &str) -> AppResult<()> {
        for r in default_regions() {
            ...
            conn.execute(
                &format!(
                    "INSERT OR IGNORE INTO region_config ({}) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    REGION_INSERT_FIELDS
                ),
                params![
                    r.name.as_str(), r.category.as_str(), "16:9", r.scroll_mode.as_str(),
                    r.grid_rows, r.grid_cols, r.overlap_x, r.overlap_y,
                    r.drag_x, r.drag_y, r.capture_region_x, r.capture_region_y,
                    r.capture_offset_y, r.template_ref.as_deref(),
                    r.target_w, r.target_h, r.sub_map.as_deref(), now, now
                ],
            )?;
        }
        Ok(())
    ...
    pub fn get_setting(&self, key: &str) -> AppResult<Option<String>> {
        ...
        let result: Option<String> = conn.query_row(
            "SELECT value FROM app_setting WHERE key = ?", params![key], |row| row.get(0),
        ).optional()?;
        Ok(result)
    }
    ...
    pub fn list_regions(&self) -> AppResult<Vec<RegionConfig>> {
        ...
        let mut stmt = conn.prepare(&format!(
            "SELECT {} FROM region_config ORDER BY category, name, scroll_mode",
            REGION_SELECT_FIELDS))?;
    ...
    fn get_region(&self, name: &str, aspect_ratio: &str, scroll_mode: &str) -> AppResult<Option<RegionConfig>> {
        let conn = self.lock_conn()?;
        let result = conn.query_row(
            &format!("SELECT {} FROM region_config WHERE name = ? AND aspect_ratio = ? AND scroll_mode = ?",
                REGION_SELECT_FIELDS),
    ...
    /// 大地图/自定义/0次：直接返回数据库值；1-8次：用 0次 target + k + rate 推导。
    pub fn derive_region_from_target(
        ...
        Ok(Some(RegionConfig {
            id: region0.id, name: region0.name.clone(),
            category: region0.category.clone(), aspect_ratio: region0.aspect_ratio.clone(),
            scroll_mode: scroll_mode.to_string(),
            grid_rows: derived.actual_rows, grid_cols: derived.actual_cols,
            overlap_x: derived.overlap_x, overlap_y: derived.overlap_y,
            drag_x: derived.drag_x, drag_y: derived.drag_y,
            capture_region_x: region0.capture_region_x, capture_region_y: region0.capture_region_y,
            capture_offset_y: region0.capture_offset_y, template_ref: region0.template_ref.clone(),
            target_w: target_w_0, target_h: target_h_0, sub_map: region0.sub_map.clone(),
            created_at: region0.created_at.clone(), updated_at: region0.updated_at.clone(),
        }))
    }
    ...
    pub fn upsert_region(&self, config: &RegionConfig) -> AppResult<()> {
        ...
        conn.execute(
            ...
            params![config.name, config.category, config.aspect_ratio, config.scroll_mode,
                ...
                config.template_ref, config.target_w, config.target_h,
                config.sub_map, now, now],
        )?;
        Ok(())
    }
    ...
    pub fn list_scroll_modes(&self) -> AppResult<Vec<ScrollMode>> {
        ...
        let rows = stmt.query_map([], |row| Ok(ScrollMode {
            id: Some(row.get(0)?), name: row.get(1)?,
            scroll_count: row.get(2)?, is_default: row.get(3)?,
        }))?;
        let mut list = Vec::new();
        for row in rows { list.push(row?); }
        Ok(list)
    ...
    pub fn load_window_state(&self) -> AppResult<WindowState> {
        ...
        let result = conn.query_row(
            ...
            [], |row| Ok(WindowState {
                x: row.get(0)?, y: row.get(1)?, width: row.get(2)?, height: row.get(3)?,
                is_maximized: row.get(4)?, always_on_top: row.get(5)?, updated_at: row.get(6)?,
            }),
        ).optional()?;
    ...
    pub fn insert_session(&self, session: &CaptureSession) -> AppResult<i64> {
        let conn = self.lock_conn()?;
        conn.execute(
            "INSERT INTO capture_session (started_at, finished_at, region, scroll_mode, grid_rows, grid_cols, total_shots, status, original_path, exported_path, thumbnail_path, crop_box, output_format, jpg_quality)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            params![session.started_at, session.finished_at, session.region, session.scroll_mode,
                session.grid_rows, session.grid_cols, session.total_shots, session.status,
                session.original_path, session.exported_path, session.thumbnail_path,
                session.crop_box, session.output_format, session.jpg_quality],
        )?;
    ...
    pub fn update_session(&self, id: i64, session: &CaptureSession) -> AppResult<()> {
        let conn = self.lock_conn()?;
        conn.execute(
            "UPDATE capture_session SET finished_at = ?, status = ?, original_path = ?, exported_path = ?, thumbnail_path = ?, crop_box = ?, output_format = ?, jpg_quality = ?, total_shots = ? WHERE id = ?",
            params![session.finished_at, session.status, session.original_path, session.exported_path,
                session.thumbnail_path, session.crop_box, session.output_format, session.jpg_quality, session.total_shots, id],
        )?;
        Ok(())
    }

    pub fn update_session_paths(&self, session_id: i64, original_path: Option<&str>,
        exported_path: Option<&str>, crop_box: Option<&str>) -> AppResult<()> {
        let now = chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string();
        let conn = self.lock_conn()?;
        // exported_path 追加（换行分隔），支持一次会话保存多张裁剪图
        conn.execute(
            "UPDATE capture_session SET
                original_path = COALESCE(?, original_path),
                exported_path = CASE
                    WHEN ? IS NULL THEN exported_path
                    WHEN exported_path IS NULL OR exported_path = '' THEN ?
                    ELSE exported_path || char(10) || ?
                END,
                crop_box = COALESCE(?, crop_box),
                finished_at = COALESCE(finished_at, ?)
             WHERE id = ?",
            params![original_path, exported_path, exported_path, exported_path, crop_box, now, session_id],
        )?;
        Ok(())
    }

    pub fn list_sessions(&self) -> AppResult<Vec<CaptureSession>> {
        let conn = self.lock_conn()?;
        let mut stmt = conn.prepare("
            SELECT id, started_at, finished_at, region, scroll_mode, grid_rows, grid_cols,
                   total_shots, status, original_path, exported_path, thumbnail_path, crop_box, output_format, jpg_quality
            FROM capture_session ORDER BY started_at DESC
        ")?;
    ...
    pub fn get_session(&self, session_id: i64) -> AppResult<Option<CaptureSession>> {
        let conn = self.lock_conn()?;
        let session = conn.query_row(
            "SELECT id, started_at, finished_at, region, scroll_mode, grid_rows, grid_cols,
                    total_shots, status, original_path, exported_path, thumbnail_path,
                    crop_box, output_format, jpg_quality
             FROM capture_session WHERE id = ?",
            params![session_id], row_to_session,
        ).optional()?;
        Ok(session)
    }

    pub fn delete_session(&self, session_id: i64) -> AppResult<()> {
        let conn = self.lock_conn()?;
        conn.execute("DELETE FROM capture_session WHERE id = ?", params![session_id])?;
        Ok(())
    }

    /// 重置所有数据：region_config/app_setting/window_state 恢复默认值；
    /// include_history=true 时同时清空 capture_session。仅清数据库记录，不删磁盘文件。
    pub fn reset_data(&self, include_history: bool) -> AppResult<()> {
        let now = chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string();
        let conn = self.lock_conn()?;
        conn.execute("DELETE FROM region_config", [])?;
        Self::insert_default_regions(&conn, &now)?;
        ...
        conn.execute("DELETE FROM window_state", [])?;
        conn.execute(
            "INSERT INTO window_state (id, width, height, is_maximized, always_on_top, updated_at) VALUES (1, 900, 600, 0, 0, ?)",
            params![now])?;
        if include_history {
            conn.execute("DELETE FROM capture_session", [])?;
        }
